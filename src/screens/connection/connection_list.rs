...
pub struct ConnectionList<'c> {
    connections: &'c Vec<ConnectionInfo>,
    selected: usize,
}

impl <'c> ConnectionList<'c> {
    pub fn new(connections: &'c Vec<ConnectionInfo>) -> Self {
        Self { connections, selected: 0 }
    }

    // renders a block for a single connection showcasing all of its info
    pub fn render_connection_block(&self, connection: &ConnectionInfo, selected: bool, area: Rect, buf: &mut Buffer) {
        let block = Block::default().title(&*connection.name)
          .borders(Borders::ALL);
...
impl <'c> EventHandler for ConnectionList<'c> {
...
impl <'c> Widget for &ConnectionList<'c> {
